...
Begin Testing
'''
Ctest = load(dataloc + '/hypy_coord.npy')
land_time = float(loadtxt(dataloc + '/hypy_land_time.txt'))
nonland_time = float(loadtxt(dataloc + '/hypy_nonland_time.txt'))
csoln = float(loadtxt(dataloc + '/hypy_curv.txt'))

nonland_avg_rel_error = float(loadtxt(dataloc + '/nonland_avg_rel_error.txt'))
landmark_rel_error = float(loadtxt(dataloc + '/landmark_rel_error.txt'))
...
# # alternative validation error using landmark distances
# D_land2nodes,  D_land, land_ids, D_nonland, nonland_ids, D_test_pairs, D_test_points = load_data_preproc(nodes, landmarks, dataloc)
# test_ans1 = zeros(len(D_test_dist0))
# test_ans2 = zeros(len(D_test_dist0))
# for ii,test_pairs in enumerate(D_test_pairs0):
# 	d0 = D_test_dist0[ii]
# 	d1 = D_land2nodes[:,test_pairs[0]]
# 	d2 = D_land2nodes[:,test_pairs[1]]
# 	min_d1 = argmin(d1)
# 	min_d2 = argmin(d2)
# 	test_ans1[ii] = d1[min_d1] + d2[min_d2] + D_land[min_d1,min_d2]
# 	test_ans2[ii] = amin(d1 + d2)
# 	# pdb.set_trace()
# val_relerr1 = linalg.norm(abs(test_ans1 - D_test_dist0))/linalg.norm(D_test_dist0)
# val_relerr2 = linalg.norm(abs(test_ans2 - D_test_dist0))/linalg.norm(D_test_dist0)
# print val_relerr1, val_relerr2
# pdb.set_trace()
