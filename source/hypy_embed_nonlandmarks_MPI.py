...
nn = args.sp
dataloc = args.s
n_split = loadtxt(dataloc + '/nonland_nsplit.txt') # total number of 

# nprocs2 = 4 # set to size
nprocs2 = size
# print "rank = ", rank
...
if rank == 0:
	...
	D0 = load(dataloc + '/D_nonland_split_' + str(nn).zfill(3) + '.npy').astype('int32')
	L_points = load(dataloc + '/land_coord_opt.npy') 
	land_err_temp_hypy = loadtxt(dataloc + '/fopt.txt')
	csoln = loadtxt(dataloc + '/copt.txt')
	curv = float(csoln)
else:
	D0 = None
	L_points = None
...
def F2(args):
	...
	# print mean(niter), mean(funcalls), mean(Fval)
	return Fval, array(X)

# compute_nonlandmarks_nn is equivalent to rank
...
inputs2 = [[seeds[i],dim,L_points,curv,nonland_rL,i,D0,node_ids[i]] for i in range(len(node_ids))]
...
f2vals = solns2[0]
x2solns = solns2[1]
save(dataloc + '/temp/' + 'x2s_split' + str(nn) + '_rank' + str(rank),x2solns)
save(dataloc + '/temp/' + 'f2s_split' + str(nn) + '_rank' + str(rank),f2vals)

end = time.time()
hypy_nonland_time = end - start
if rank == 0: print "    **Parallel split took %.2f seconds" %(hypy_nonland_time)
savetxt(dataloc + '/temp/' + 'nonland_time_split' + str(nn) + '_rank' + str(rank) + '.txt',[hypy_nonland_time])
